...
@patch("app.main.is_rate_limited", side_effect=_no_rate_limit)
...
@patch("app.main.is_rate_limited", side_effect=_always_rate_limit)
...
def test_jwt_payload_contains_required_claims(mock_rl, client: TestClient) -> None:
    ...
    # exp must be in the future and respect JWT_EXPIRY_MINUTES
    expected_exp = payload["iat"] + settings.JWT_EXPIRY_MINUTES * 60
